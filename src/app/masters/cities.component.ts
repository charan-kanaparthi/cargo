...
export class CitiesComponent implements OnInit 
{
  ...
  loadAPI:Promise<any>;
  isForm=true;
  componentName="Cities";//to display on the page
  tfields=["Id","name","code","stateId"];//table fields
  private zone: NgZone;
  ...
  constructor(private router: Router,private mastersService: MastersService,private http:Http,@Inject(Renderer) private renderer: Renderer) 
  {
      ...
      this.theads.push('Id','Name','Code','State ID','Action');
  }
  ...
  { 
       ...
       for(let field of this.fields)
      {
       ...
       alert(JSON.stringify(form.value));
      this.mastersService.add(form.value,"cities");
  }
  ...
  {  
       ...
       for(let field of this.modalfields)
      {
       ...
       alert(JSON.stringify(upform.value));
      this.mastersService.update(upform.value,"cities");
  }
