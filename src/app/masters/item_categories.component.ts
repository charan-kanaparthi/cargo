...
export class Item_CategoriesComponent implements OnInit 
{
  ...
  tfields=["Id","name","description"];//table fields
  private zone: NgZone;
  ...
  constructor(private router: Router,private mastersService: MastersService,private http:Http,@Inject(Renderer) private renderer: Renderer) 
  {
      ...
      //table fields
      this.theads.push('id','Name','Description','Action');
  }
  ...
  getItemCategories() 
  {
    this.mastersService.getData("item_categories").subscribe((data)=>{setTimeout(()=>{this.data=data}, 2000)});
  }
