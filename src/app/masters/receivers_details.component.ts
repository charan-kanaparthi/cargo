...
export class Receivers_DetailsComponent implements OnInit 
{ 
  sId:any;
  fields:Field[]=[];
  theads:String[]=[];
  ...
  table_data:any[]=[];
  tfields=["Id","idType","idNo","name","emailId","mobileNumber","landline","city","block","commune","address","tinNumber","n"];
  tformFields=["Id","idType","idNo","name","emailId","mobileNumber","landline","city","block","commune","address","tinNumber","n","receiverNo","senderId","status"];
  private zone: NgZone;
  ...
  constructor(private router: Router,private mastersService: MastersService,private http:Http,@Inject(Renderer) private renderer: Renderer, public Activatedroute: ActivatedRoute) 
  {
   ...
   //   alert("id--"+this.sId);
      ...
      this.fields.push(test);
      test={name:'operation',id:'',  value:'update',type:'hidden',label:'',isRequired:true,isReadOnly:false,isSelected:false,className:'form-control',options:null ,chckboxvalues:null};
      this.fields.push(test);
      ...
      this.fields.push(test);
      test={name:'senderId',id:'',  value:'',type:'hidden',label:'',isRequired:true,isReadOnly:false,isSelected:false,className:'form-control',options:null ,chckboxvalues:null};
      this.fields.push(test);
...
//receiverModalFields
      ...
      this.deletemodalfields.push(test);
      this.theads.push("ID","ID Type","ID No","Name","Email ID","Mobile Number","Landline","City","Block","Commune","Address","TIN Number","N","Action");
       //var table_row={};
...
public  tabulateForm(item:any)
{
   var receiverNo=0;
   this.selectedTabledata=item; 
    for (let ref of this.fields) 
    {
              for(let tff of this.tformFields)
              { 
                if(ref['name']==tff && ref['name']!='checkbox' )
...
modalTodefault(){
 for (let ref of this.receivermodalfields) 
 {
  ...
  getCustomers() 
  {
      this.mastersService.getDatabyId("customers", this.sId).subscribe((data)=>{setTimeout(()=>{
      this.data=data;
      for(var i in this.data)
      {
        if(this.data[i].senderId==0){
            this.tabulateForm(this.data[i]);
        }
        else{
        this.fillTableData(this.data[i]);
...
public fillTableData(item:any){
   item['receiverNo']=this.receiverNos;
   item['status']="DELETED";
